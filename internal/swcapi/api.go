...
func (s *PublicBlockChainAPI) BlockNumber() uint64 {
	block, _ := s.b.BlockByNumber(context.Background(), rpc.LatestBlockNumber) // latest header should always be available

	fmt.Println("block height", block.Height)
	return block.Height.Uint64()
}

// GetBalance returns the amount of wei for the given address in the state of the
// given block number. The rpc.LatestBlockNumber and rpc.PendingBlockNumber meta
// block numbers are also allowed.
func (s *PublicBlockChainAPI) GetBalance(ctx context.Context, address common.Address,blockNr  rpc.BlockNumber) (int64, error) {
	...
	return s.b.GetBalance(ctx,address)
}
...
// transaction hashes.
func (s *PublicBlockChainAPI) rpcOutputBlock(b *core.Block, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	//head := b.Header() // copies the header once
	fields := map[string]interface{}{
		//"number":           (*hexutil.Big)(b.Height),
		"hash":             b.Hash.Bytes(),
		"parentHash":      b.PrevBlockHash.Bytes(),
		"nonce":            b.Nonce,
		...
		"size":             hexutil.Uint64(len(b.Serialize())),
...
func newRPCTransaction(tx *core.Transaction, blockHash common.Hash, blockNumber uint64, index uint64) *RPCTransaction {
	var signer core.Signer = core.FrontierSigner{}
	/*if tx.Protected() {
		signer = types.NewEIP155Signer(tx.ChainId())
	}*/
	from, _ := core.Sender(signer, tx)
...
func newRPCPendingTransaction(tx *core.Transaction) *RPCTransaction {
	return newRPCTransaction(tx, common.Hash{}, 0, 0)
}

// newRPCTransactionFromBlockIndex returns a transaction that will serialize to the RPC representation.
func newRPCTransactionFromBlockIndex(b *core.Block, index uint64) *RPCTransaction {
	txs := b.Transactions
...
func newRPCRawTransactionFromBlockIndex(b *core.Block, index uint64) hexutil.Bytes {
	txs := b.Transactions
	if index >= uint64(len(txs)) {
		return nil
	}
...
func newRPCTransactionFromBlockHash(b *core.Block, hash common.Hash) *RPCTransaction {
	for idx, tx := range b.Transactions {
		if tx.CommonHash() == hash {
...
func (args *SendTxArgs) toTransaction(wallet core.Wallet,b Backend) *core.Transaction {
	var input []byte
	if args.Data != nil {
		input = *args.Data
	} else if args.Input != nil {
	...
	/*if args.To == nil {
		return core.NewContractCreation(uint64(*args.Nonce), (*big.Int)(args.Value), uint64(*args.Gas), (*big.Int)(args.GasPrice), input)
	}*/
...
func (s *PublicTransactionPoolAPI) PendingTransactions() ([]*RPCTransaction, error) {
	...
	for _, tx := range pending {
		//var signer core.Signer = core.HomesteadSigner{}
		/*if tx.Protected() {
			signer = core.NewEIP155Signer(tx.ChainId())
		}*/
		//from, _ := core.Sender(signer, tx)
