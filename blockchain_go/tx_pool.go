...
func (uts *TxPool) InitDB() error {
	err := uts.Bc.Db.Update(func(tx *bolt.Tx) error {
...
func (uts *TxPool) ForEach(callback ForEachKeyIteratorInterface) error {
	return uts.Bc.forEachInBucket(TxPoolBucket, callback)
}

// get count of records in the table
func (uts *TxPool) GetCount() (int, error) {
	return uts.Bc.getCountInBucket(TxPoolBucket)
}*/

func (uts *TxPool) TruncateDB() error {
	err := uts.Bc.Db.Update(func(tx *bolt.Tx) error {
		err := tx.DeleteBucket([]byte(TxPoolBucket))
...
func (uts *TxPool) GetTransaction(txID []byte) ([]byte, error) {
	var txBytes []byte

	err := uts.Bc.Db.View(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(TxPoolBucket))
...
func (uts *TxPool) PutTransaction(txID []byte, txdata []byte) error {
	return uts.Bc.Db.Update(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(TxPoolBucket))
...
func (uts *TxPool) insert(tx *Transaction) error {
	return uts.PutTransaction(tx.ID,tx.Serialize())
}
...
func (uts *TxPool) DeleteTransaction(txID []byte) error {
	return uts.Bc.Db.Update(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(TxPoolBucket))
...
*/
	...
	if old != nil {
		pool.all.Remove(old.CommonHash())
		/*pool.priced.Removed()
*/
...
func (pool *TxPool) lockedReset(oldHead, newHead *Block) {
	pool.mu.Lock()
...
func (pool *TxPool) reset(oldHead, newHead *Block) {
	// If we're reorging an old state, reinject all dropped transactions
	var reinject Transactions

	if oldHead != nil && oldHead.Hash != newHead.PrevBlockHash {
		...
		if depth := uint64(math.Abs(float64(oldNum) - float64(newNum))); depth > 64 {
			...
			var rem,err = pool.Bc.GetBlockByHashNumber(oldHead.Hash.Bytes(), oldHead.Height.Uint64())
			if(rem == nil ||err != nil){
				panic(err)
			}
			var add,err1 = pool.Bc.GetBlockByHashNumber(newHead.Hash.Bytes(), newHead.Height.Uint64())
			if(add == nil ||err1 != nil){
				panic(err1)
			}
			...
			for rem.NumberU64() > add.NumberU64() {
				...
				if rem,err = pool.Bc.GetBlockByHashNumber(rem.PrevBlockHash.Bytes(), rem.NumberU64()-1); rem == nil || err !=nil {
			...
			for add.NumberU64() > rem.NumberU64() {
				...
				if add,err = pool.Bc.GetBlockByHashNumber(add.PrevBlockHash.Bytes(), add.NumberU64()-1); add == nil || err !=nil{
...
func (pool *TxPool) Content() (map[common.Address]Transactions, map[common.Address]Transactions) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
...
// will apply.
func (pool *TxPool) AddRemotes(txs []*Transaction) []error {
...
func (pool *TxPool) add(tx *Transaction, local bool) (bool, error) {
	...
	if list := pool.pending[from]; list != nil && list.Overlaps(tx) {
		...
		go pool.txFeed.Send(NewTxsEvent{Transactions{tx}})
...
func (pool *TxPool) enqueueTx(hash common.Hash, tx *Transaction) (bool, error) {
	...
	inserted, old := pool.queue[from].Add(tx, uint64(0))
	if !inserted {
		// An older transaction was better, discard this
